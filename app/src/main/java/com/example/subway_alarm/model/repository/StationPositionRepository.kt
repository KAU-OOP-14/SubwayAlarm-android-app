...
class StationPositionRepository : FirebaseRepository {
    ...
    override suspend fun postSelectedId(
        selectedPos: PointF,
        scale: Float,
        transValue: PointF,
        stationId: NonNullMutableLiveData<Int>
    ) {
        withContext(Dispatchers.IO) {
            println("position이 repository에 post 되었습니다!")
            var id: Int = 0
            val originX: Float =
                (selectedPos.x / scale) + (1080 * (scale - 1) / (2 * scale)) - transValue.x
            val originY: Float =
                (selectedPos.y / scale) + (2280 * (scale - 1) / (2 * scale)) - transValue.y
